    ...
    async ({ step }) => {
        ...
        for (const budget of budgets) {
            ...
            await step.run(`check-budget-${budget.id}`, async () => {
                ...
                // TODO: Change back to 80 after testing!
                if (percentageUsed >= 75 && (!budget.lastAlertSent || isNewMonth(new Date(budget.lastAlertSent), new Date()))) {
                    //Send Email
    ...
    async ({ step }) => {
        ...
        for (const user of users) {
            await step.run(`generate-report-${user.id}`, async () => {
                ...
                if (!emailResponse.success) {
                    throw new Error(
                        `Failed to send email to ${user.email}: ${emailResponse.error}`
                    );
                }
            })
        }

        return { proessed: users.length }
    }
)
...
async function generateFinancialInsights(stats, month) {
    ...
    const prompt = `
        ...
        - Total Income: BDT ${stats.totalIncome}
        - Total Expenses: BDT ${stats.totalExpenses}
        - Net Income: BDT ${stats.totalIncome - stats.totalExpenses}
        - Expense Categories: ${Object.entries(stats.byCategory)
            .map(([category, amount]) => `${category}: BDT ${amount}`)
            .join(", ")}
